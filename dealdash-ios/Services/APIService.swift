...
class APIService {
    ...
    func fetchDeals(
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            session.request(url, parameters: params, encoding: URLEncoding.default)
                .validate()
                .responseData { resp in
                    if let data = resp.data {
                        do {
                            let deals = try Config.jsonDecoder.decode([Deal].self, from: data)
                        ...
                        continuation.resume(throwing: NSError(domain: "APIService", code: -1, userInfo: [NSLocalizedDescriptionKey: "Unknown error"]))
                    }
                }
        }
    }
